				...
				<div class="row g-4">
					...
					<div class="col-xl-3 text-end">
						<a href="/myrecipe/create"
							class="btn border-secondary py-3 px-4 rounded-pill text-primary bg-white">
				...
				<div class="row g-4 mt-2">
									...
									<c:forEach items="${dto}" var="d">
										<tr>
											...
											<td><p class="mb-0 mt-4">
													<a href="/myrecipe/detail?recipeNum=${d.recipeNum}">${d.recipeTitle}</a>
												</p></td>
					...
					<div class="col-12">
						<div class="pagination d-flex justify-content-center mt-5">
							<a
								href="./myList?page=${pager.pre?pager.start-1:pager.start}&search=${pager.search}&kind=${pager.kind}"
								class="rounded ${pager.pre ? '' : 'disabled'}">&laquo;</a>

							<c:forEach begin="${pager.start}" end="${pager.end}" var="i">
								<a
									href="./myList?page=${i}&search=${pager.search}&kind=${pager.kind}"
									class="rounded">${i}</a>
							</c:forEach>

							<a
								href="./myList?page=${pager.next?pager.end+1:pager.end}&kind=${pager.kind}&search=${pager.search}"
