                ...
                <div class="col-lg-7 mx-auto">
                    ...
                    <!-- 파일삭제버튼 클릭시 db에서 삭제되기전에 변수에담아 컨트롤러로 넘겨줌(로컬에서도 삭제하기위해) -->
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="recipeTitle" value="${dto.recipeTitle}" class="w-100 form-control border-0 py-3 mb-4">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">내용</label>
                            <textarea name="recipeContents" class="w-100 form-control border-0 py-3 mb-4" rows="10" style="resize: none;">${dto.recipeContents}</textarea>
                        </div>
                       
                        <div class="mb-4">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <label class="form-label mb-0">요리 사진 관리 (기존+신규 최대 5개)</label>
                                <button type="button" id="btn-add-file" class="btn btn-sm btn-secondary">
                                    <i class="fas fa-plus"></i> 새 파일 추가
                                </button>
                            </div>
                            
                            <div id="existing-file-container" class="mb-3">
                                <c:if test="${not empty dto.recipeFileDTO and not empty dto.recipeFileDTO[0].fileName}">
                                    ...
                                    <div class="row g-2 mb-3">
                                        <c:forEach items="${dto.recipeFileDTO}" var="fileDTO">
                                            <div class="col-md-4 col-sm-6 existing-file-group">
                                                <div class="position-relative border rounded overflow-hidden bg-white text-center p-2" style="height: 120px;">
                                                    <img src="${pageContext.request.contextPath}/files/${name}/${fileDTO.fileName}" class="img-fluid h-100" style="object-fit: cover;" alt="${fileDTO.oriName}">
                                                    <button type="button" class="btn btn-danger btn-sm btn-delete-existing position-absolute top-0 end-0 m-1" data-file-num="${fileDTO.fileNum}">✕</button>
                                                </div>
                                            </div>
                                        </c:forEach>
                                    </div>
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <a href="/myrecipe/detail?recipeNum=${dto.recipeNum}" class="w-100 btn form-control border-secondary py-3 bg-white text-secondary text-center">취소</a>
                        </div>
                    </form>
                </div>
            </div>
